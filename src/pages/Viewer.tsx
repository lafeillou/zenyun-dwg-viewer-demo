...
export function Viewer() {
	...
	function dragged(event, d) {
		...
		console.log(event.y);
		console.log(event);
		const [x, y] = currentTransform.current.invert([event.sourceEvent.clientX, event.sourceEvent.clientY]);
		console.log(x, y)
	...
	return (
      ...
      direction="horizontal"
			...
			style={{border: '1px solid #efefef', borderRadius: 8, padding:'0px 0px 0px 24px'}}
			onLayout={handleLayoutChange}
    >
      <ResizablePanel defaultSize={25}>
        <div className="grid w-full max-w-sm items-center gap-3" style={{marginTop: 24, minWidth: 280}}>
					<Label htmlFor="picture">请上传一个DWG格式文件</Label>
					<Input id="picture" type="file" accept=".dwg" onChange={handleInputChange}/>
				</div>
				<div id="jsoneditor" style={{minWidth: 280, marginRight: 24, marginTop: 24, overflow: 'auto',
    height: 'calc(100vh - 180px)'}}></div>
      </ResizablePanel>
      <ResizableHandle withHandle style={{border: '1px solid #efefef'}}/>
      <ResizablePanel defaultSize={75}>
        <div className="flex h-full items-center justify-center p-6" id="svg-container" style={{display: 'none'}}>
        </div>
